...
- New NF → copy `nf/_template/` and follow its **§0 New NF Checklist** verbatim.
  A new NF MUST be aligned with the existing ones and build with zero errors in
  every environment. Non-negotiables: it is part of the **root module** (no per-NF
  `go.mod`; imports use `github.com/francurieses/5gc-rel17/…`), its Dockerfile keeps
  the template shape (`COPY go.mod go.sum* ./` + `COPY shared/`, `ARG GO_VERSION=1.26.2`),
  and after any new dependency you run `GOWORK=off go mod tidy` from the repo root.
  Wire it into all four surfaces — `.github/workflows/ci.yml` docker matrix,
  `docker-compose.yml` (service + PCAP sidecar, `profiles: [core]`), root `Makefile`
  `NFS :=` list — then prove it with `GOWORK=off go build ./nf/... ./shared/...` and
  `make -C nf/<name> docker` before pushing.
