Feature: Nlmf_Location DetermineLocation — Cell-ID positioning (TS 29.572 §5.2.2.2, TS 23.273 §7.2)
  ...
  # ---------------------------------------------------------------------------
  # Scenario 4 — UE CM-IDLE: AMF returns 409/504 UE_NOT_REACHABLE (spec deviation)
  # TS 23.273 §7.2 — NGAP relay requires CM-CONNECTED; CM-IDLE → failure (paging deferred)
  # Error table: UE CM-IDLE → UE_NOT_REACHABLE
  # ---------------------------------------------------------------------------
  Scenario: DetermineLocation for a CM-IDLE UE returns UE_NOT_REACHABLE
