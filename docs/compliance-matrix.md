...
| Location Services — Nlmf_Location DetermineLocation (Cell-ID MVP) | TS 23.273 §6, §7.2 / TS 29.572 §5.2.2.2, §6.1.6.2.2 | **LMF (new)**, AMF | 🟡 Cell-ID positioning MVP. `POST /nlmf-loc/v1/ue-contexts/{id}/provide-loc-info` → consumes Namf_Location → maps NRCGI→LocationData (locationEstimate POINT, nrCellId, tai, ageOfLocationEstimate). lat/lon=0 placeholder (config NRCGI→coord map; documented MVP simplification). NRF reg nfType=LMF + service nlmf-loc (§6.1.6.3.3). Causes: MANDATORY_IE_MISSING→400, CONTEXT_NOT_FOUND→404, LOCATION_FAILURE/UE_NOT_REACHABLE→504. metric fivegc_lmf_locate_total{result}. Deferred (LMF-002+): LPP/NRPPa, EventSubscription, CancelLocation, GMLC/N56, UDM privacy check, paging-then-locate for CM-IDLE | LMF-001 | ✅ |
| Namf_Location ProvideLocationInfo (AMF producer) | TS 29.518 §5.2.2.6 | AMF, LMF | 🟡 `POST /namf-loc/v1/ue-contexts/{id}/provide-loc-info` on :8001 SBI. Resolves UE, requires CM-CONNECTED, relays NGAP LocationReportingControl, blocks on pendingLoc chan (10 s impl-defined guard, no normative timer), returns LocationData {nrCellId, tai}. Causes: MANDATORY_IE_MISSING→400, CONTEXT_NOT_FOUND→404, **CM-IDLE→409 UE_NOT_REACHABLE** (LMF client remaps to 504 for LCS consumer), LOCATION_FAILURE→504. Deferred: paging-then-locate, last-known-location | LMF-001 | ✅ |
